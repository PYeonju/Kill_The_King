...
void CObjectManager::LineToWallCheck(std::vector<CObjectBase*> pool)
{
	...
	if (player->m_isCollidedWhileDash)
		return;
	...
	for (int i = 0; i < pool.size(); i++)
	{
		...
		case eMyStatus::AFTERDASHUPLEFT:
		{
			if ((player->m_DashPos.y <= rect.bottom) && (player->m_DashPos.x <= rect.right));
			{
				...
				// (Right, y)
				float y1 = 1 * (rect.right - player->m_pos.x) + player->m_pos.y;

				float x1 = 1 * (rect.bottom - player->m_pos.y) + player->m_pos.x;

				if (y1 <= rect.bottom && rect.top <= y1)
					rightvalue = true;
				...
				// 충돌안함
				if ((rightvalue && bottomvalue) || (!rightvalue && !bottomvalue))
					return;
				...
				else
				{
					if (rightvalue)
					{
						...
						player->m_DashPos.y = y1;
						player->m_isCollidedWhileDash = true;
						return;
					}
					else if (bottomvalue)
					{
						player->m_DashPos.x = x1;
